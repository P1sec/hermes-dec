...
class SyntaxFlags(IntFlag):
    ...
    UNICODE = (
        1 << 3
    )  # UNICODE, later shorteneted to UCODE was added in version 0.3.0.
...
def get_opcodes_enum(bytecode_version: int):
    ...
    opcode_name_and_min_version: List[Tuple[str, int]] = [
        ...
        (
            'BeginLoop' if bytecode_version >= 79 else 'BeginLoopPre79',
            MIN_BC_VERSION,
        ),
...
def decompile_regex(regex: ParsedRegex):
    ...
    pending_lookaround_instructions: List[
        Union[LookaroundInst, LookaheadInst]
    ] = []
    pending_beginloop_instructions: List[
        Union[BeginLoopInst, BeginLoopPre78Inst]
    ] = []

    for instruction in regex.instructions:
        while (
            instruction.original_pos in alternation_pos_to_jump_target.values()
        ):
        ...
        if isinstance(instruction, BeginMarkedSubexpressionInsn):
            ...
            display_brackets = display_brackets and not (
                ...
                and instruction.brackets[0].start
                == instruction.brackets[0].end
            )
            if display_brackets:
                output += '['
            ...
            for bracket in instruction.brackets:
                if bracket.start == bracket.end:
                    ...
                    output += escape_range(
                        chr(bracket.start) + chr(bracket.end)
                    )
                else:
                    output += (
            ...
            if (
                ...
                not in secondary_branch_to_alternation_pos
            ):
                output += '(?:'
                secondary_branch_to_alternation_pos[
                    instruction.secondaryBranch
                ] = instruction.original_pos
            else:
                secondary_branch_to_alternation_pos[
                    instruction.secondaryBranch
        ...
        # =
            ...
            raise NotImplementedError(
                'Please disassemble opcode: %r' % instruction
            )
