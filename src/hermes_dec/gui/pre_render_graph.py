...
def draw_stuff(
    instructions: List[ParsedInstruction], dehydrated: DecompiledFunctionBody
) -> dict:
    ...
    def process_basic_block(basic_block: BasicBlock):
        ...
        start_idx = dehydrated.instruction_boundaries.index(
            basic_block.start_address
        )
        end_idx = dehydrated.instruction_boundaries.index(
        ...
        child_nodes = sorted(
            basic_block.child_nodes + basic_block.error_handling_child_nodes,
            key=lambda block: block.start_address,
        )  # -block.max_acc_insn_weight,
        # ^ TODO: Reintegrate this opti

        child_nodes = list(
            filter(
                lambda block: (
                    (not block.rendered) and (not block.marked_to_render)
                ),
                child_nodes,
            )
        )
    ...
    for index, basic_block in enumerate(block_objects):
        ...
        result['blocks'][index]['child_error_nodes'].extend(
            block_objects.index(node)
            for node in basic_block.error_handling_child_nodes
        )
        ...
        result['blocks'][index]['parent_error_nodes'].extend(
            block_objects.index(node)
            for node in basic_block.error_handling_parent_nodes
        )
