...
class ProjectInstanceManager:
    ...
    web_console_backlog: List[
        Dict[str, str]
    ]  # List of logged "indexing_state" or "indexing_status_log" socket messages
    sockets: List[
    ...
    def new_with_name(cls, file_hash: str, name: str):
        ...
        symlink(
            join('..', 'by-date', datebased_name),
            self.subdir_path,
        )

        self.file_buffer = open(
            self.subdir_path + '/index.android.bundle', 'wb+'
        )
    ...
    def open_by_hash(cls, file_hash):
        ...
        self.file_buffer = open(
            self.subdir_path + '/index.android.bundle', 'rb'
        )
    ...
    async def monitor_subprocess_stderr(self, child):
        ...
        while True:
            ...
            await self.broadcast_message(
                {'type': 'console_error_log', 'data': data}
            )
    ...
    async def monitor_subprocess_stdout_or_exit(self, child):
        ...
        while True:
            ...
            if data['type'] == 'indexing_state':
                ...
                await self.broadcast_message(
                    {'type': 'console_log', 'data': raw_message}
                )

            elif data['type'] == 'indexing_status_log':
                await self.broadcast_message(
                    {'type': 'console_log', 'data': data['message']}
                )
    ...
    async def launch_index_subprocess_task(self):
        ...
        self.child = await create_subprocess_exec(
            sys.executable,
        ...
        async with self.file_parsing_lock:
            if not self.reader:
                await get_running_loop().run_in_executor(
                    None, self.parse_file_blocking
                )

                await self.launch_index_subprocess_task()

                self.database_index = DatabaseIndexReader(
                    self.reader, self.subdir_path
                )
            ...
            filter(
                ...
                [
                    ...
                    sub(
                        r'[^a-z0-9\-]',
                        '',
                        sub(r'\s+', '-', unidecode(orig_name).lower().strip()),
                    ),
                ],
            )
        )
