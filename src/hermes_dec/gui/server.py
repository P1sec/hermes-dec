...
class WebsocketClient:
    ...
    def create_file(self, file_hash: str, file_name: str):
        self.project = ProjectInstanceManager.new_with_name(
            file_hash, file_name
        )
    ...
    def get_metadata(self) -> dict:
        ...
        return {'file_metadata': self.project.read_metadata()}
            ...
            async for msg in self.socket:
                ...
                if type(msg) == bytes:
                    ...
                    if msg_type == 'open_file_by_hash':
                        if ProjectInstanceManager.project_hash_exists(
                            msg['file_hash']
                        ):
                            if self.project:
                                await self.project.unregister_socket(
                                    self.socket
                                )
                        ...
                        await self.socket.send(
                            dumps(
                                {'type': 'file_opened', **self.get_metadata()}
                            )
                        )
                            ...
                            dumps(
                                {
                                    ...
                                    **table_model.get_json_response(
                                        ...
                                        current_page_if_not_current_row=msg.get(
                                            'page'
                                        ),
                        ...
                        function_header = self.reader.function_headers[
                            msg['function_id']
                        ]

                        instructions = list(
                            parse_hbc_bytecode(function_header, self.reader)
                        )
                        ...
                        dehydrated.is_global = (
                            msg['function_id']
                            == self.reader.header.globalCodeIndex
                        )

                        if dehydrated.function_object.hasExceptionHandler:
                            dehydrated.exc_handlers = (
                                self.reader.function_id_to_exc_handlers[
                                    msg['function_id']
                                ]
                            )
                        ...
                        await self.socket.send(
                            dumps(draw_stuff(instructions, dehydrated))
                        )
