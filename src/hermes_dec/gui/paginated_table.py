...
class StringsList(TableModel):
    ...
    def query_rows(
        ...
        if search_query and search_query.strip():
            matching_string_ids: List[int] = (
                server_connection.search_index.find_strings_from_substring(
                    search_query.strip()
                )
            )
        ...
        if self.has_pagination:
            ...
            current_page = min(
                page_count, current_page_if_not_current_row or 1
            )

            displayed_string_ids = matching_string_ids[
                (current_page - 1) * self.pagination_thresold : current_page
                * self.pagination_thresold
            ]
        ...
        displayed_rows: List[Dict[str, Any]] = [
            {'id': string_id, 'cells': (str(string_id), strings[string_id])}
            for string_id in displayed_string_ids
        ]  # [{'id': 49, 'cells': ['a', 'b', 'c']}, ...]
    ...
    columns = [ColumnModel('Id', 'id'), ColumnModel('Name', 'name')]
...
class FunctionsList(TableModel):
    ...
    def query_rows(
        ...
        # Do search query filtering here
        if search_query and search_query.strip():
            matching_function_ids: List[int] = (
                server_connection.search_index.find_functions_from_substring(
                    search_query.strip()
                )
            )
        ...
        displayed_rows: List[Dict[str, Any]] = [
            {
                'id': function_id,
                'cells': (
                    strings[headers[function_id].functionName]
                    or 'fun_%08x' % headers[function_id].offset,
                    '%08x' % headers[function_id].offset,
                    str(headers[function_id].bytecodeSizeInBytes),
                ),
            }
            for function_id in displayed_function_ids
        ]  # [{'id': 49, 'cells': ['a', 'b', 'c']}, ...]
...
class HeaderInfo(TableModel):
        ...
        displayed_rows = [
            ...
            for index, cells in enumerate(
                [
                    ...
                    (
                        'String section size',
                        format_size(header.stringStorageSize),
                    ),
                ]
            )
        ]
    ...
    columns = [ColumnModel('Field', 'field'), ColumnModel('Value', 'value')]
