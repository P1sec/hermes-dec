...
from serialized_literal_parser import (
    unpack_slp_array,
    SLPArray,
    SLPValue,
    TagType,
...
class DatabaseIndexGenerator:
    def __init__(self, reader: HBCReader, dir_path: str):
        ...
        print(
            dumps({'type': 'indexing_state', 'state': 'fully_indexed'}),
            flush=True,
        )
    ...
    def indexate_xrefs(self):
        with self.Session() as session:
            ...
            if (
                not meta_block
                or meta_block.database_format < XREF_DB_FORMAT_VERSION
            ):
                print(
                    dumps(
                        {
                ...
                for function_count, function_header in enumerate(
                    self.reader.function_headers
                ):
                    for insn in parse_hbc_bytecode(
                        function_header, self.reader
                    ):
                        for operand_index, operand in enumerate(
                            insn.inst.operands
                        ):
                            if operand.operand_meaning:
                                operand_value = getattr(
                                    insn, 'arg%d' % (operand_index + 1), None
                                )
                                if operand_value is not None:
                                    if (
                                        operand.operand_meaning
                                        == OperandMeaning.string_id
                                    ):
                        ...
                        if insn.inst.name in (
                            'NewArrayWithBuffer',
                            'NewArrayWithBufferLong',
                        ):
                            for item in unpack_slp_array(
                                self.reader.arrays[insn.arg4 :], insn.arg3
                            ).items:
                                if item.tag_type in (
                        ...
                        elif insn.inst.name in (
                            'NewObjectWithBuffer',
                            'NewObjectWithBufferLong',
                        ):
                            for item in unpack_slp_array(
                                self.reader.object_keys[insn.arg4 :], insn.arg3
                            ).items:
                                if item.tag_type in (
                                    TagType.LongStringTag,
                                    TagType.ShortStringTag,
                            ...
                            for item in unpack_slp_array(
                                self.reader.object_values[insn.arg5 :],
                                insn.arg3,
                            ).items:
                                if item.tag_type in (
                                    TagType.LongStringTag,
