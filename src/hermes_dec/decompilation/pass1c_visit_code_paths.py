...
class CycleDetectorAlgState:
    ...
    branches: List[
        'CodeExplorationPath'
    ]  # Contains references to parents with fork points
...
class VisitableNode:
    ...
    def do_initial_linking(self):
        self.unvisited_children = set(
            ...
            for unwrapped_item in self.wrapped.child_nodes
            + self.wrapped.error_handling_child_nodes
        )
    ...
    def match_against_child(self, other_node):
        ...
        will_create_cycle = (
            other_node in other_node.current_tip_of.children[:-1]
        )
...
class CodeExplorationPath:
    ...
    def copy_merge(self, other_branch: 'Self'):
        non_common_descents = [
            node for node in self.children if node not in other_branch.children
        ]

        other_branch.children = (
